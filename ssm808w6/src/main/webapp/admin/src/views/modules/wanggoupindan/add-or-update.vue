<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="拼单折后价" prop="pindanzhehoujia">
          <el-input v-model="ruleForm.pindanzhehoujia" 
              placeholder="拼单折后价" clearable  :readonly="ro.pindanzhehoujia"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="拼单折后价" prop="pindanzhehoujia">
              <el-input v-model="ruleForm.pindanzhehoujia" 
                placeholder="拼单折后价" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `wanggoupindan/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.wanggoupindanCrossAddOrUpdateFlag = false;
                  this.parent.search();
